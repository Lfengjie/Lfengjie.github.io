...
### 读流程
- Image::aio_write2处于Librbd里面，在librbd的c++接口中，这个接口可用来写入数据，其它的接口如：Image::aio_write, rbd_aio_read等函数也可用来写入数据，其在调用流程上大同小异
```
...
- ImageRequestWQ<I>::aio_write处于 ImageRequestWQ中，程序会根据用户的配置自动选择将image请求放入队列中还是直接调用。ImageRequestWQ初始化时会启动一个线程专门用于处理队列中的数据，具体的入口函数为ImageRequestWQ<I>::process()
